...
附注应修改之处：
1.一个是深度问题，即如果我们将深度大于等于四层的时候处理不了，输出invalid的时侯怎么办？
我认为ab算法函数里的for (auto& mv : moves)这步的moves不需要全部遍历。
但最终如何解决还得团队研究。
2.另一个可修改的地方，但有点复杂，即形成活三后，跳冲四的局面如何判断的问题，是否需要判断？
3.判断双方分数差的时候，可将两个函数合并成一个函数，虽然差别不大。